...
namespace PBB.Businesslayer.Render.WAV
{
    /// <summary>
    ...
    public class WAVFile
    {
        ...
        public String Open(WAVFileMode pMode)
        {
            ...
            if (!System.IO.File.Exists(filenameBackup))
            ...
            try
            {
                mFileStream = System.IO.File.Open(mFilename, FileMode.Open);
                mFileMode = pMode;
        ...
        public void Create(String pFilename, bool pStereo, int pSampleRate, short pBitsPerSample, bool pOverwrite)
        {
            ...
            try
            {
                ...
                if (pOverwrite)
                    mFileStream = System.IO.File.Open(pFilename, FileMode.Create);
                else
                    mFileStream = System.IO.File.Open(pFilename, FileMode.CreateNew);
        ...
        static public bool IsWaveFile(String pFilename)
        {
            ...
            if (System.IO.File.Exists(pFilename))
            {
                try
                {
                    FileStream fileStream = System.IO.File.Open(pFilename, FileMode.Open);
        ...
        private static double amountDone;

        public static double GetAmountDone()
        {
            return amountDone;
        }
