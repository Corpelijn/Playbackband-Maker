using PBB.Businesslayer.Exceptions;
using System;
...
namespace PBB.Businesslayer
{
    ...
    public class Playbackband
    {
        ...
        public static Playbackband Instance { get; set; }
        ...
        {
            get { return this.blokken; }
            set { this.blokken = value; }
        }
        ...
        public void AddBlok(string naam, int aantalNummers)
        {
            blokken.Add(new Blok(naam, aantalNummers, (blokken.Count + 1), this));
        }
        ...
        public int ResolveGlobalFragmentIDToBlokID(int fragmentID)
        {
            int[] indexes = GetBlokAndFragmentIDFromGlobalFragmentID(fragmentID);

            return indexes[0];
        }

        public int ResolveGlobalFragmentIDToFragmentID(int fragmentID)
        {
            int[] indexes = GetBlokAndFragmentIDFromGlobalFragmentID(fragmentID);
        ...
        private int[] GetBlokAndFragmentIDFromGlobalFragmentID(int id)
        {
            int blokindex = 0;
            while (id >= blokken[blokindex].Fragmenten.Count)
            {
                id -= blokken[blokindex].Fragmenten.Count;
                blokindex++;
            }

            return new int[] { blokindex, id };
        }

        public Fragment GetFragment(int index)
        {
            int[] indexes = GetBlokAndFragmentIDFromGlobalFragmentID(index);

            return blokken[indexes[0]].Fragmenten[indexes[1]];
        }

        public Blok GetBlok(int index)
        {
            return blokken[index];
        }

        public Blok[] GetBlokken()
        {
            return blokken.ToArray();
        }

        public int GetBlokkenCount()
        {
            return blokken.Count;
        }
        ...
        public static void LoadPlaybackband(string filename)
        {
            File f = new File(filename);
            Playbackband.Instance = f.Open();
            Playbackband.Instance.file = f;
        }

        public static void CreateNewPlaybackband(string filename)
        {
            // Maak de nieuwe playbackband aan
            Playbackband.Instance = new Playbackband(filename);
